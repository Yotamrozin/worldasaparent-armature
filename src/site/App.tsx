...
// (see photoPositions.ts) — resolved server-side against each image's
// original filename and baked into props already-positioned, so there's no
// client-side lookup that could go wrong on hydration.
...
import type { PhotoSlot } from './photoPositions';
...
// Hero/writing-sample images arrive pre-positioned (see photoPositions.ts +
// index.astro), unlike every other image on the page.
export interface PositionedImageProp extends ImageProp, PhotoSlot {}
...
export interface AppProps {
  hero: {
    title: string;
    byline?: string;
    images: PositionedImageProp[];
  };
  writingSample: {
    heading: string;
    intro?: string;
    bandImages: PositionedImageProp[];
    ...
    >
      {hero.images.map((img, i) => (
        <ParallaxImage
          ...
          factor={img.factor}
          reveal
          revealDelay={img.revealDelay ?? i * 0.12}
          className="absolute rounded-sm shadow-sm"
          style={isMobile ? img.mobile : img.desktop}
        />
      ))}
  ...
  return (
    <section id="writing-sample" className="bg-white pt-0">
      <div className="relative h-80 md:h-[30rem] w-full">
        {writingSample.bandImages.map((img, i) => (
          <ParallaxImage
            key={i}
            src={img.url}
            alt={img.alt}
